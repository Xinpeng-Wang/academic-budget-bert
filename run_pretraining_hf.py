...
from pretraining.base import BasePretrainModelHuggingFace
...
def prepare_model_and_optimizer(args):
    ...
    model = BasePretrainModelHuggingFace(args)
...
def prepare_distillation_optimizer(args):
     ...
     # Load Pre-training Model skeleton + supplied model config
    student = BasePretrainModelHuggingFace(args)
    if args.student_initialize is not None:
        student.network.from_pretrained(args.student_initialize, args=None)
    ...
    teacher = BertModel.from_pretrained(args.teacher_path, args=None)
    teacher.to(args.device)
