...
def att_val_kl(student_atts, students_vals, teacher_atts, teacher_vals, layer_selection):

    loss_att = 0.
    loss_value = 0.

    batch_head_size, length, dk = student_atts[0].shape
    ...
    for student_att, teacher_att in zip(student_atts, new_teacher_atts):
        ...
        loss_kl_tmp = F.kl_div(torch.log(student_att), teacher_att, reduction='sum')/ (batch_head_size * length) #, reduction='batchmean', log_target=True)
        loss_att += loss_kl_tmp

    new_teacher_value = [teacher_vals[i] for i in layer_selection]

    for student_value, teacher_value in zip(students_vals, new_teacher_value):
